...
def print_schedule(schedule, num_days, max_lessons_per_day, num_classes, class_names):
    ...
    for class_index in range(num_classes):
        for lesson_index in range(max_lessons_per_day):
            lessons = [schedule[class_index][day_index][lesson_index] if lesson_index < len(
                schedule[class_index][day_index]) else Lesson(" ", Teacher(" ", []), " ") for day_index in
                       range(num_days)]
