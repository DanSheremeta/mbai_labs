...
for i in range(10):
    ...
    best_path, best_path_length = ant_colony_optimization(points, n_ants=n_ants, n_iterations=100, alpha=alpha,
                                                          beta=beta, evaporation_rate=evaporation_rate, Q=Q)
    print(
        f"Iteration {i + 1}: Best Path = {best_path},\n Best Path Length = {best_path_length},Number o ants = {n_ants}, alpha= {alpha}, beta= {beta}")
    ...
    for j, point in enumerate(points):
        plt.text(point[0], point[1], f'{j + 1}')
